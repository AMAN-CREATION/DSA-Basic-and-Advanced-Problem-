...
//     int ans = INT_MAX;
//     for (int k = i + 1; k < j; k++)
//         ans = min(ans, v[i] * v[j] * v[k] + solveMemo(v, i, k) + solveMemo(v, k, j));
...
int minScoreTriangulation(vector<int> &value)
{
    ...
    // return solveMemo(value, 0, n - 1, dp);

    return solveTabu(value);
}

int main()
{
    vector<int> value = {1, 2, 3};
    int score = minScoreTriangulation(value);
    cout << "The smallest total possible score is " << score << endl;
}